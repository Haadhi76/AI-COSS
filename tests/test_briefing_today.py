...
def test_today_force_regenerates_and_overwrites_row(client, monkeypatch):
    from services import claude_service
    ...
    first = client.post("/api/briefing/today", json={"messages": SAMPLE_MESSAGES}).json()
    forced = client.post(
        "/api/briefing/today?force=true", json={"messages": SAMPLE_MESSAGES}
    ).json()

    assert calls["n"] == 4  # Claude is called again for triage + briefing.
    assert forced["id"] == first["id"]  # Row is updated in place, not duplicated.
    assert forced["completed_ids"] == []  # State resets after regeneration.
    assert forced["overrides"] == {}
    assert forced["day_summary"] is None
